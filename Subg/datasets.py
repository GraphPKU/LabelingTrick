...
class BaseGraph(Data):
    def __init__(self, x, edge_index, edge_weight, subG_node, subG_label,
        ...
        super(BaseGraph, self).__init__(x=x,
                                        ...
                                        y=subG_label)
        self.mask = mask
        num_node = self.x.shape[0]
        num_subg, subgsize = self.pos.shape[0], self.pos.shape[1]
        row = torch.arange(num_subg).unsqueeze_(-1).expand(-1, subgsize).flatten()
        col = self.pos.flatten()
        mask = col>=0
        row = row[mask]
        col = col[mask]
        self.node2subg = SparseTensor(row=row, col=col, sparse_sizes=(num_subg, num_node))
        self.to_undirected()
    ...
    def to(self, device):
        ...
        self.node2subg = self.node2subg.to_device(device)
        return self
