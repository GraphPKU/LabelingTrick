...
def train(mod, x, ei, boosted_deg, hop:int, opt, pos_positive, pos_negative):
    ...
    adj = SparseTensor(row=segei[0], col=segei[1], value=segea, sparse_sizes=(seg_num_node, seg_num_node)).to_device(device)
    pred = mod(x.to(device), adj, segpos.to(device))
...
def test(mod, x, ei, boosted_deg, hop: int, dl: Utils.tensorDataloader, score_fns, limit: int = -1):
    ...
    for i, batch in enumerate(dl):
        ...
        segea = 1/torch.sqrt(segea)
        adj  = SparseTensor(row=segei[0], col=segei[1], value=segea, sparse_sizes=(seg_num_node, seg_num_node)).to_device(device)
        preds.append(mod((t_deg-1).to(device) if x is None else (x[node_idx]).to(device) , adj, segpos.to(device)))
        ys.append(batch[-1])
...
def work(num_step: int = 100000,
    ...
    trn_posidx_dl = iter(
        tensorDataloader([torch.arange(split_edge["train"]["edge"].shape[1])],
                         batch_size, True, True, torch.device("cpu")))
    trn_negpos_dl = iter(
    ...
    tst_score = 0.0
    early_stop = 0
    full_deg = degree(edge_index[0], num_node, torch.long)
    boosted_deg = full_deg + 1
    full_ei = edge_index
    full_x = x
    split_edge["train"]["edge"] = split_edge["train"]["edge"]
    best_mod = None
...
if args.test:
    ...
    for rep in range(10):
        set_seed(rep)
        ret = work(**p, do_test=True, test_interval=interval, max_early_stop=4.01*interval, num_step=50*interval)
