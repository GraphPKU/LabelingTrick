...
from impl.Utils import to_directed, edge2ds, tensorDataloader
import argparse
import torch
...
num_node = max(
    torch.max(edge_index).item() + 1, x.shape[0] if x is not None else 0)
split_edge = None
...
def split():
    ...
    if not is_directed:
        edge_index = torch.cat(
            (split_edge["train"]["edge"], split_edge["train"]["edge"][:, [1, 0]]),
            dim=-1).reshape(-1, 2).t()
...
@torch.no_grad()
def test(mod, x, adj, dl: Utils.tensorDataloader, score_fns, limit: int = -1):
    mod.eval()
    preds = []
    ys = []
    for i, batch in enumerate(dl):
        if i >= limit and limit >= 0:
            break
        preds.append(mod(x, adj, *batch[:-1]))
        ys.append(batch[-1])
...
def buildModel(hid_dim: int, num_mplayer: int, labelmod0: str, labelmod1: str,
    ...
    if labelmod0 == "None":
        ...
        RepreMod = model.PLabelingNet2Set(convs, f0s, f1s)
    ...
    return model.FullModel(max_x, num_node, head_act, RepreMod, PredMod,
                           emb_dim)
...
def work(num_step: int = 100000,
         ...
         do_test: bool = True,
         **kwargs):
    val_dl, tst_dl = (tensorDataloader((pair[0].t(), pair[1]), batch_size,
                                       False, True, device)
    ...
    tst_score = 0.0
    early_stop = 0
    full_adj = Utils.eiea2adj(*Utils.gcn_norm(edge_index, num_node),
                              num_node).cuda()  #to_device(device)
    full_x = degree(edge_index[0], num_node,
                    torch.long).to(device) if x is None else x
    split_edge["train"]["edge"] = split_edge["train"]["edge"].to(device)
    best_mod = None
    for iteration in range(num_step):
        if (iteration+1) % test_interval == 0:
            val_score = test(mod, full_x, full_adj, val_dl, [score_fn],
                             test_interval)[0]
        ...
        degree_t = degree(edge_t[0], num_node, torch.long) if x is None else x
    ...
    if do_test:
        ...
        tst_score = test(best_mod.to(device), full_x, full_adj, tst_dl, [score_fn])[0]
...
def search(trial: optuna.Trial):
    ...
    return work(**fixed_params,
                ...
                dp0=dp0,
                ln1=ln1,
                ln2=ln2, num_step=2000, max_early_stop=200, test_interval=20)[0]
...
if args.test:
    ...
    for rep in range(10):
        set_seed(rep)
        ret = work(**p, **fixed_params, do_test=True)
