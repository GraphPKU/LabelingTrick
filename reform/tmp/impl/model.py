...
from torch_sparse import SparseTensor, masked_select_nnz, masked_select
from typing import Iterable, Optional
from .labelutils import distslabel, distslabel_sparse, zolabels, pprlabel
import functools
...
class PlabelNetOneHead(nn.Module):
    num_layer: Final[int]
    num_nodes: Final[int]
    max_dist: Final[int]
    res: Final[str]
    ...
    def __init__(self,
        ...
        if x is not None:
            ...
            self.xlin = nn.Sequential(nn.Linear(x.shape[1], hid_dim))
            print("use_x")
        else:
            self.use_x = False
        assert res in ["add", "cat", "no"]
        if label_fn == "ppr":
            assert num_bin is not None
        self.num_bin = num_bin
        self.res = res
        self.distemb = nn.Embedding(max_dist + 10 if num_bin is None else num_bin+10, hid_dim)
        self.label_aggr = nn.Linear((max_dist+1)*hid_dim, hid_dim) if label_fn == "ppr" else nn.Identity()
        self.econvs = nn.ModuleList([GCNConv(hid_dim, hid_dim, decomposed_layers=2) for i in range(num_layer)])
        self.num_layer = num_layer
        self.mlps = nn.ModuleList([
            nn.Sequential(
                nn.ReLU(inplace=True), nn.Linear(hid_dim, hid_dim),
                nn.LayerNorm(hid_dim), nn.ReLU(inplace=True))
            for _ in range(num_layer)
        ])
               
        self.output = nn.Linear(num_layer * hid_dim if self.res == "cat" else hid_dim, 1)
        self.num_nodes = num_nodes
        self.max_dist = max_dist
        
        if label_fn == "dist":
            self.label_fn = distslabel
        elif label_fn == "zo":
            self.label_fn = zolabels
        elif label_fn == "ppr":
            self.label_fn = functools.partial(pprlabel, num_bin=self.num_bin)
        else:
            raise NotImplementedError

    def getdist(self, ei: Tensor, head: Tensor)->Tensor:
        '''
        dist = []
        for i in head:
            dist.append(distlabel(i, self.max_dist, ei, self.num_nodes))
        return torch.stack(dist)
        '''
        
        return self.label_fn(head, self.max_dist, ei, self.num_nodes)

    def forward(self, ei: Tensor, head: Tensor)->Tensor:
        dist = self.getdist(ei, head)
        x: Tensor = self.distemb(dist)
        if x.dim() > 3:
            x = x.flatten(-2, -1)
        x = self.label_aggr(x)
        if self.use_x:
            x = x + self.xlin(self.x).unsqueeze(0)
        if self.res == "add":
            for i in range(self.num_layer):
                x = self.mlps[i](self.econvs[i](x, ei)) + x
        elif self.res == "cat":
            xs = []
            for i in range(self.num_layer):
                x = self.mlps[i](self.econvs[i](x, ei))
                xs.append(x)
...
from .mygcnconv import myGCNConv
from torch_geometric.nn import MessagePassing
import torch_sparse
from torch_geometric.utils import degree

class PlabelNet_directed(nn.Module):
    num_layer: Final[int]
    num_nodes: Final[int]
    max_dist: Final[int]
    res: Final[str]
    ...
    def __init__(self,
        ...
        if x is not None:
            self.use_x = True
            self.register_buffer("x", x)
            self.xlin = nn.Sequential(nn.Linear(x.shape[1], hid_dim), nn.ReLU(inplace=True))
        else:
            self.use_x = False
        if label_fn == "ppr":
            assert num_bin is not None
        ...
        self.distemb = nn.Embedding(max_dist + 10 if num_bin is None else num_bin+10, hid_dim)
        self.label_aggr = nn.Linear((max_dist+1)*hid_dim, hid_dim) if label_fn == "ppr" else nn.Identity()
        self.econv1 = myGCNConv(hid_dim, hid_dim, **kwargs)
        self.num_layer = num_layer
        self.mlps = nn.ModuleList([nn.Sequential(nn.ReLU(inplace=True), nn.Linear(hid_dim, hid_dim)) for _ in range(num_layer)])
        self.output = nn.Sequential(nn.ReLU(inplace=True), nn.Linear(hid_dim, 1))
        self.num_nodes = num_nodes
        self.max_dist = max_dist
        
        if label_fn == "dist":
            self.label_fn = distslabel_sparse
        elif label_fn == "zo":
            self.label_fn = zolabels
        elif label_fn == "ppr":
            self.label_fn = functools.partial(pprlabel, num_bin=self.num_bin)
    ...
    def forward(self, ei: SparseTensor, head: Tensor)->Tensor:
        if self.to_un:
            ei = ei.to_symmetric()
        dist = self.getdist(ei, head)
        x: Tensor = self.distemb(dist)
        ...
        if self.res == "add":
            for i in range(self.num_layer):
                x = self.mlps[i](self.econv1(x, ei)) + x
        elif self.res == "cat":
            xs = []
            for i in range(self.num_layer):
                x = self.mlps[i](self.econv1(x, ei))
                xs.append(x)
            x = torch.cat(xs, dim=-1)
        elif self.res == "no":
            for i in range(self.num_layer):
