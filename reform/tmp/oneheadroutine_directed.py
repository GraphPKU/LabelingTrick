...
from impl.Utils import do_edge_split, to_directed
import argparse
import torch
...
import myDataloader
from myDataloader import trainDirDataloader, testDirDataloader, trainDirSparseTensorDataloader
'''
...
def split():
    ...
    split_edge["valid"]["edge_neg"] = split_edge["valid"]["edge_neg"].to(
        device)
...
def test(mod, adj, dl: myDataloader.testDataloader, score_fns):
    ...
    for src, tar in dl:
        ...
        pred = mod(adj, src).squeeze(-1)
        preds.append(torch.gather(pred, 1, tar).cpu())
    pred = torch.cat(preds, dim=0)
    return tuple((score_fn(pred[:, 0], pred[:, 0:]) for score_fn in score_fns))


from impl.labelutils import train_negsample_sparse
...
def train(mod, adj, opt, tar_edge, num_neg: int, temperature: float):
    ...
    negbatch = train_negsample_sparse(adj, tar_edge, num_neg, num_node)
...
def buildModel(hid_dim: int, num_layer: int, **kwargs):
    return model.PlabelNet_directed(num_node, hid_dim, num_layer, **kwargs)
...
def work(
    ...
    trn_dl = trainDirSparseTensorDataloader(split_edge["train"]["edge"].cuda(), batch_size, num_node)
    val_dl = testDirDataloader(split_edge["valid"]["source_node"].cuda(),
                               split_edge["valid"]["target_node"].cuda(),
                               split_edge["valid"]["target_node_neg"].cuda(),
                               batch_size)
    tst_dl = testDirDataloader(split_edge["test"]["source_node"].cuda(),
                               split_edge["test"]["target_node"].cuda(),
                               split_edge["test"]["target_node_neg"].cuda(),
                               batch_size)
    ...
    for epoch in range(num_epoch):
        ...
        for step, batch in enumerate(trn_dl):
            ...
            if step % log_step == 0:
                print(
                    f"{int(time()-t0)} loss {losslist[-1]} smooth {smooth_loss}",
                    flush=True)
            if step >= sub_sample:
                break
...
if __name__ == "__main__":
    ...
    if args.dataset.startswith("ogbl"):
        ...
        def evaluate_mrr(pospred, negpred):
            '''
            pred (B, 1)
            y (B),  idx
            '''
